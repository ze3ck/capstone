...
async function llenarTablaGestionUsuario() {
  ...
  for (let x of data.response) {
    // console.log(x.EMAIL);
    guardarCambiosGestion(x.EMAIL, x.ESTADO, x.ID_USUARIO, x.NOMBRE_USUARIO);
  }
  ocultarLoader();
}
// document.addEventListener("DOMContentLoaded", function () {
...
function guardarCambiosGestion(EMAIL, ESTADO, ID_USUARIO, NOMBRE_USUARIO) {
  ...
  dropdown.id = "dropdown_" + ID_USUARIO; // Asignar un id único basado en el usuario
  ...
  tr.appendChild(col2);
  tr.appendChild(col3);
  tr.appendChild(col4); // Añadir el dropdown aquí
